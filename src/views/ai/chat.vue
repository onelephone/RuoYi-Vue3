...
async function handleSendMessage() {
  ...
  console.log('[SSE] Authorization:', 'Bearer ' + token)

  // 超时兜底：若流在 STREAM_TIMEOUT_MS 内未结束（后端 Flux 未正常 complete），
  // 主动 abort 连接，确保 isStreaming 复位、输入框恢复可用（避免永久卡死）
  const STREAM_TIMEOUT_MS = 120000
  const abortController = new AbortController()
  const timeoutTimer = setTimeout(() => abortController.abort(), STREAM_TIMEOUT_MS)
  ...
  try {
    ...
    while (true) {
      ...
      // 流自然结束（Flux 完成）：主动复位，不再依赖后端的 event: done
      if (done) {
...
function renderMarkdown(text) {
  ...
  // 表格：匹配连续的 | 开头行块（表头 + 分隔行 + 数据行）
  html = html.replace(/((?:^\|.*\|\s*\n?)+)/gm, (block) => {
    const rows = block.trim().split('\n').map(r =>
      r.replace(/^\||\|$/g, '').split('|').map(c => c.trim())
    )
    if (rows.length < 2) return block  // 不足两行不是表格，原样保留
    const header = rows[0]
    const body = rows.slice(2)  // 跳过分隔行（rows[1] 为 ---|---）
    let table = '<table class="md-table"><thead><tr>'
    table += header.map(c => `<th>${c}</th>`).join('')
